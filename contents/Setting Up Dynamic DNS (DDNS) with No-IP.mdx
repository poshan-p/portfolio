...
A pop-up window will appear (Screenshot 4). Enter your desired hostname, and select one of the free domain options available (I chose `ddns.net` and used `cyberstorm-example` as the hostname). Leave the IPv4 address field empty if it automatically detects your current IP; otherwise, input it manually.
